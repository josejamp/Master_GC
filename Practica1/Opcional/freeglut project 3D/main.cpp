...
void buildSceneObjects() {
	...
	/* IMPORTANTE, poner a true para que se dibuje
	   USAR LOS BOTONES 1 y 2 RESPECTIVAMENTE*/
	hipo = true, car = false;

	hipotrocoide = new Hipotrocoide(10, 1000);
...
void key(unsigned char key, int x, int y){
	...
	switch (key) {
		...
		case 'g': if(hipo) hipotrocoide->setModo(GL_POLYGON); break;
		case 'h': if(hipo) hipotrocoide->setModo(GL_LINE_LOOP); break;
		case 'f': if(car) coche->desplazaAdelante(); break;
		case 'v': if(car) coche->desplazaAtras(); break;
		case '1': {
			hipo = true;
			car = false;
		} break;
